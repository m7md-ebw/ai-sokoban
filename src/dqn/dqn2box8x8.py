import os, math, argparse, logging, warnings
...
NUM_BOXES = 2
IMG_SIZE = 128
ENV_ID_STR = f"Sokoban(8x8,{NUM_BOXES}boxes)"
...
# Preprocessing (gray, 128x128, UINT8) + framestack
    ...
    if isinstance(infos, (list, tuple)):
        for i, inf in enumerate(infos):
            if isinstance(inf, dict):
                ...
                all_on[i] = ao or (nbi >= 2)
                if dones[i] and (ao or nbi >= 2):
                    solved_at_done[i] = 1
        ...
        if fi is not None:
            for i, inf in enumerate(fi):
                if isinstance(inf, dict) and dones[i]:
                    ...
                    solved_at_done[i] = 1 if (ao or nbi >= 2) else 0

    all_on = np.maximum(all_on, (nb >= 2).astype(np.int32))
...
class TrainingVideoBuffer:
    ...
    def save_env_episode(self, env_idx, path):
        frames = self._frames[int(env_idx)]
...
def parse_args():
    p = argparse.ArgumentParser(
        f"Vectorized DQN on {ENV_ID_STR} — dueling CNN + shaping + training-video + logfile"
    )
    ...
    p.add_argument("--train-video-dirname", type=str, default="videos_train/dqn8x8", help="subdir inside logdir to store MP4s")

    p.add_argument("--max-steps-per-episode", type=int, default=140, help="TimeLimit cap for 8x8 levels")
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--cpu", action="store_true")

    p.add_argument("--logdir", type=str, default="runs/sokoban_8x8_2boxes_dqn_vec")
    p.add_argument("--checkpoint-dir", type=str, default="checkpoints/dqn8x8")
    p.add_argument("--checkpoint-every", type=int, default=200)
    p.add_argument("--logfile", type=str, default="train_log8x8.txt")
    return p.parse_args()
