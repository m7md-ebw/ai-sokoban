...
NUM_BOXES = 1                # 1 box instead of 2
IMG_SIZE = 112               # original size for Sokoban-small-v0
...
# Env helpers (Sokoban-small-v0)

def make_single_env(seed, max_steps=None):
    def thunk():
        try:
            env = gym.make(ENV_ID, num_boxes=NUM_BOXES)
        except TypeError:
            env = gym.make(ENV_ID)
            try:
                un = env.unwrapped
                patched = False
                for attr in ("num_boxes", "n_boxes", "_num_boxes"):
                    if hasattr(un, attr):
                        try:
                            setattr(un, attr, NUM_BOXES)
                            patched = True
                            break
                        except Exception:
                            pass
                if patched:
                    ...
                    logging.getLogger("sokoban").warning(
                        f"[WARN] Could not set num_boxes={NUM_BOXES} on env {ENV_ID}; "
                    )
            except Exception:
                logging.getLogger("sokoban").warning(
                    "[WARN] Unexpected error while trying to patch env for num_boxes; continuing with default env."
                )
...
# Preprocessing (gray, 112x112, UINT8) + framestack
...
class ReplayBuffer:
    def __init__(self, cap: int, k: int, h: int, w: int):
        ...
        self.s  = np.empty((self.cap, self.k, self.h, self.w), dtype=np.uint8)
        self.ns = np.empty((self.cap, self.k, self.h, self.w), dtype=np.uint8)
    ...
    if isinstance(infos, (list, tuple)):
        for i, inf in enumerate(infos):
            if isinstance(inf, dict):
                ...
                all_on[i] = ao or (nbi >= NUM_BOXES)
                if dones[i] and (ao or nbi >= NUM_BOXES):
                    solved_at_done[i] = 1
        ...
        if fi is not None:
            for i, inf in enumerate(fi):
                if isinstance(inf, dict) and dones[i]:
                    ...
                    solved_at_done[i] = 1 if (ao or nbi >= NUM_BOXES) else 0

    all_on = np.maximum(all_on, (nb >= NUM_BOXES).astype(np.int32))
...
def parse_args():
    p = argparse.ArgumentParser(
        "Vectorized DQN on Sokoban-small-v0 (dueling + k4 + signed shaping + finish bonus + training-video + logfile)"
    )
    ...
    p.add_argument("--train-video-dirname", type=str, default="runs/dqn1box/videos", help="subdir inside logdir to store MP4s")

    p.add_argument("--max-steps-per-episode", type=int, default=120, help="TimeLimit cap; keeps env id the same")
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--cpu", action="store_true")

    p.add_argument("--logdir", type=str, default="runs/dqn1box")
    p.add_argument("--checkpoint-dir", type=str, default="runs/dqn1box/ckpt")
    p.add_argument("--checkpoint-every", type=int, default=200)
    p.add_argument("--logfile", type=str, default="traindqn1box.txt")
    return p.parse_args()
