...
class UndoMovePenaltyWrapper(_gym.Wrapper):
    # Sokoban actions are 0:push_up,1:push_down,2:push_left,3:push_right,4:up,5:down,6:left,7:right
...
def _construct_sokoban(dim_room, num_boxes, max_steps):
    ...
    raise RuntimeError(
        "Could not construct a Sokoban env."
        "unexpected env ID."
    )
...
def make_base_env(dim_room=(7, 7), num_boxes=1, max_steps=120, step_penalty=0.0, print_tag=None):
    ...
    env = NoOpMovePenaltyWrapper(env, penalty=0.05)     # change value using CLI   
